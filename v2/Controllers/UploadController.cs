...
public sealed class UploadController : ControllerBase
{
    ...
    [HttpPost("direct-form")]
    [DisableRequestSizeLimit]
    [RequestFormLimits(MultipartBodyLengthLimit = 1L * 1024 * 1024 * 1024)]
    public async Task<IActionResult> DirectForm([FromForm] IFormFile file, CancellationToken ct)
    {
        if (file is null || file.Length == 0)
            return BadRequest(new { error = "Arquivo ausente ou vazio." });
        ...
        var (success, error, publicUri) = await _direct.UploadAsync(
            stream,
            file.FileName,
            file.ContentType ?? "application/octet-stream",
            ct);
        ...
        if (!success)
            return Problem(title: "Falha no upload direto (IFormFile)", detail: error, statusCode: 500);
