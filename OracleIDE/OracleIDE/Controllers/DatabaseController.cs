...
public class DatabaseController : ControllerBase
{
    ...
    public async Task<ActionResult<ConnectionResult>> Connect([FromBody] ConnectionRequest req)
    {
        try
        {
            var version = await _connManager.ConnectAsync(
                req.Host, req.Port, req.ServiceName, req.User, req.Password);
    ...
    public async Task<ActionResult<ConnectionResult>> TestConnection([FromBody] ConnectionRequest req)
    {
        try
        {
            var version = await _connManager.TestConnectionAsync(
                req.Host, req.Port, req.ServiceName, req.User, req.Password);
        ...
        catch (Exception ex)
        {
            ...
            {
                Success = false,
                Message = $"Falha na conexão: {ex.Message}"
            });
        }
    }
        ...
        try
        {
            ...
            if (queryType == "SELECT")
            {
                ...
                var msg = count >= maxRows
                    ? $"Exibindo primeiras {maxRows} linhas (limite). Query executada em {sw.ElapsedMilliseconds}ms"
